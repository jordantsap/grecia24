...
/*someguy start*/
				...
				if ($field->required == 1)
					$return .= "<input class='adsmanager_required' mosReq='1' id='$name' type='text' mosLabel='$strtitle' name='$name' size='$field->size' maxlength='$field->maxlength' $read_only value='$value' onkeypress=\"if(this.value.match(/\D/)) this.value=this.value.replace(/\D/g,'')\"; onkeyup=\"if(this.value.match(/\D/)) this.value=this.value.replace(/\D/g,'')\"; />\n"; 
				else
					$return .= "<input class='adsmanager' id='$name' type='text' name='$name' mosLabel='$strtitle' size='$field->size' maxlength='$field->maxlength' $read_only value='$value' onkeypress=\"if(this.value.match(/\D/)) this.value=this.value.replace(/\D/g,'')\"; onkeyup=\"if(this.value.match(/\D/)) this.value=this.value.replace(/\D/g,'')\"; />\n";
...
/*someguy end*/
	...
	//***********  Edit By eXnet : 3/8/2012 ************** MAD 20/09/12 ***//
	function showFieldForm_vExnet($field,$content,$default,$__Show_field_name = "",$__show_title = "",$__show_only_title = false)
	{
		$return = "";
		
		if( $field->name != $__Show_field_name && $__Show_field_name != "" ){
			return 	$return;
		}
			
		$strtitle = JText::_($field->title);
		if( $__show_title == "show_title" ){
			if (isset($strtitle)) {
				$return .= "".$strtitle."\n";
				$strtitle = htmlspecialchars($strtitle);
				/*MAD*/
				if ($__show_only_title) {
					return 	$return;
				}
				/*MAD*/
			}
...
/*someguy start*/
				...
				if($name == 'ad_orofosnumber'){
					
					if ($field->required == 1)
						$return .= "</b></span><div ><input  class='adsmanager_required' mosReq='1' id='$name' type='text' mosLabel='$strtitle' name='$name' size='$field->size' maxlength='$field->maxlength' $read_only value='$value' onkeypress=\"if(this.value.match(/\D/)) this.value=this.value.replace(/\D/g,'')\"; onkeyup=\"if(this.value.match(/\D/)) this.value=this.value.replace(/\D/g,'')\"; /></div>\n"; 
					else
						$return .= "</b></span><div ><input ' class='adsmanager' id='$name' type='text' name='$name' mosLabel='$strtitle' size='$field->size' maxlength='$field->maxlength' $read_only value='$value' onkeypress=\"if(this.value.match(/\D/)) this.value=this.value.replace(/\D/g,'')\"; onkeyup=\"if(this.value.match(/\D/)) this.value=this.value.replace(/\D/g,'')\"; /></div>\n";
...
/*someguy end*/
	...
	function showFieldValue_MAD($content,$field, $show_title = true, $show_value = true)
	{		
		$return = "";
		if ((strpos($field->catsid, ",".$content->catid.",") !== false)||(strpos($field->catsid, ",-1,") !== false))
		{
			if (($field->type != 'checkbox')&&(($field->display_title & $this->modetitle) == $this->modetitle))
			{
				if ($show_title == true) {
						if($field->fieldid=='42'){
								$return .= '<span id="rent_type" class="field_name">'.JText::_($field->title).': </span>';
						}
						elseif($field->fieldid=='83'){
								$return .= '<span id="rent_time" class="field_name">'.JText::_($field->title).': </span>';
						}
						elseif(($field->fieldid=='35') || ($field->fieldid=='65')){
							$return .= '';
						}
						elseif(($field->fieldid=='74') || ($field->fieldid=='73') || ($field->fieldid=='75') ){
							$return .= '<span class="field_name" style="float:left;width:100%;font-weight:bold;font-size:16px;">'.JText::_($field->title).'</span>';
						}
							elseif(($field->fieldid=='2') || ($field->fieldid=='1') || ($field->fieldid=='76') ){
							$return .= '<span class="field_name">'.JText::_($field->title).': </span>';
						}
						else{
							$return .= '<span class="field_name">'.JText::_($field->title).": ".'</span>';
						}
			...
			switch($field->type)
			{
				case 'checkbox':
					...
					{
						$return .= '<b>'.JText::_($field->title).'</b>'; //."<br />";
					}		
					break;
				...
				$t=0;
						...
						if (strpos($value, ",".$fieldvalue.",") !== false)
						{
							if ($field->type == 'multicheckbox'){
								...
								if($field->fieldid=='65'){
									...
									$return .= '<span>* '.JText::_($fieldtitle).'</span>';
								}
							
								elseif(($field->fieldid=='35') || ($field->fieldid=='74') || ($field->fieldid=='73') || ($field->fieldid=='75')) {
									
									if($t==0){
										if(($field->fieldid=='74') || ($field->fieldid=='73') || ($field->fieldid=='75') ) {
											$return .= '<ul style="width:100%;float:left;">';
										}
										elseif(($field->fieldid=='35') ) {
											$return .= '<ul style="width:31%;float:left;">';
										}
										
									}
									$return .= '<li>* '.JText::_($fieldtitle).'</li>';
							...
							//$return .= "<br />";
							if(($field->fieldid=='35') || ($field->fieldid=='74') || ($field->fieldid=='73') || ($field->fieldid=='75')||($field->fieldid=='65')) {
									$return .= '';
								}else{
							$return .= ", ";
								}
					...
					$return = rtrim($return, ", "); //bazoume , se kathe pedio kai sto telos koboume to teleutaio , //MAD
				...
				case 'number':
					...
					break;
				case 'text':
							if($field->fieldid=='77'){$return .= 'ru'.$value;}else{$return .= $value;};
					break;
				case 'price':
					if ($value != "") {	
						$return .= sprintf(JText::_('ADSMANAGER_DEVICE'),number_format(floatval($value), 2, '.', ' '));
