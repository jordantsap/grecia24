...
$content->id = $id; /*MAD*/
	...
	function delete()
	{
		...
		$app->redirect( JRoute::_('index.php?option=com_adsmanager&view=list&Itemid='.$itemid), JText::_('ADSMANAGER_CONTENT_REMOVED') );
	}

	function sendmessage()
	{
		//return print_r(JRequest::get());
		//require_once('recaptchalib.php');
		//$privkey = "6LfnXasUAAAAACqSlxcpO-qG3saDMONrUrG3Ck_R";//"6Lc1siITAAAAAGwil6OQx_AAASVs5P_4jhvv5n2o";
		...
		// if(!$resp->is_valid){
   	// 	 $app->redirect( JRoute::_('index.php?option=com_adsmanager&view=details&catid='.$content->catid.'&id='.$contentid.'&Itemid='.$itemid), "wrong captcha" );
  	// 	}
  	// 	else{
		...
		if (isset($content)){
			...
			$body = str_replace(array("\r\n", "\n", "\r"), "<br />", $body);
			$body_message = 'Grecia24.ru '.$subject.'<br/>Name:'.$name.'<br/>Email:'.$email.'<br/>Phone:'.$phone.'<br/>Message:'.$body;
			Mail($content->email,$subject,$body_message,$headers);
			Mail($adminemail,$subject,$body_message,$headers);
		}
		...
		/*if (isset($content))
		{
			...
			$body = JRequest::getVar('body' , "" );
			$body = str_replace(array("\r\n", "\n", "\r"), "<br />", $body);
			$body_message = 'Grecia24.ru '.$subject.'<br>'.$name.'<br>'.$email.'<br>'.$phone.'<br>'.$body;
			...
			if ($file['tmp_name'] != "")
			{
				...
				if (!JUtility::sendMail($email,$name,$content->email,$subject,$body_message,1,NULL,NULL,$filename))
				{
					$this->setError(JText::_('ADSMANAGER_ERROR_SENDING_MAIL ds'));
					$app->redirect( JRoute::_('index.php?option=com_adsmanager&view=details&catid='.$content->catid.'&id='.$contentid.'&Itemid='.$itemid), JText::_('ADSMANAGER_ERROR_SENDING_MAIL sd') );
				}
			}
			else {
				if (!JUtility::sendMail($email,$name,$content->email,$subject,$body_message,1))
				{
					$this->setError(JText::_('ADSMANAGER_ERROR_SENDING_MAIL 12'));
					$app->redirect( JRoute::_('index.php?option=com_adsmanager&view=details&catid='.$content->catid.'&id='.$contentid.'&Itemid='.$itemid), JText::_('ADSMANAGER_ERROR_SENDING_MAIL 12') );
				}
			}
		}*/
